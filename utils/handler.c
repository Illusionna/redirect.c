...
int coroutine_server(void *args) {
    ...
    // Set port reuse.
    if (socket_setopt(C, SOL_SOCKET, SO_REUSEADDR, NULL, 0) == SOCKET_INVALID) {
...
int run_server(char *app_name, int port) {
    ...
    while (PROXY_SERVICE_RUNNING) {
        ...
        if (threadpool_add(pool, http_redirect, (void *)(intptr_t)c, 1, NULL)) {
...
int add_route(char *app_name, char *old_url, char *new_url) {
    ...
    if (new_url == NULL) {
        char hash[33];
        md5_string(old_url, hash);
        snprintf(buffer, 1024, "ADD:%s|/%s", old_url, truncate_string(hash, 6));
    } else {
...
char *truncate_string(char *str, int n) {
